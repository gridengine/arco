 ...
 ************************************************************************/
/*___INFO__MARK_END__*/
package com.sun.grid.arco.validator.common;
...
public class CommonValidator implements Validator {
   ...
   public void validate(QueryType query, QueryStateHandler handler) {      
      if( query.isSetName() ) {
         ...
         handler.setSaveable( name != null && name.length() > 0);
      } else {
         handler.setSaveable( false);
      }
      handler.setConvertable(ArcoConstants.SIMPLE.equals(query.getType()));
   }
